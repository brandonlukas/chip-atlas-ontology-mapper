...
- **Stage 7: retrieval quality fix** — NEXT
  - Surfaced during a post-Stage-6 smoke test on 7 real Matcha parquet rows. Review-mode inspection showed two distinct retrieval failure modes; both route the LLM into correct-but-useless abstention because the candidate list contains zero valid answers.
  - **Failure mode A — non-cell ontologies contaminate the index.** For `Pancreatic islets`, top-10 is 7× `PR:*` proteins + 2× `HGNC:*` genes + 1× enzyme. For `CD4+ T cells`, top-10 is 5× CD protein variants + 2× gene symbols + 1× assay. `UBERON:0000006` and `CL:0000624` never surface. Root cause: the FAISS corpus indexes every EFO term, but EFO imports whole ontologies (PR ~17k, OBA ~17k, MONDO ~16k, HGNC ~6k) that are never valid answers to "what cell / tissue / condition was this assay run in". The PubMedBert embedder then ranks them highly because their labels lexically overlap the query.
  - **Failure mode B — subtype dominance on broad queries.** `Lung` → `UBERON:0002299` (alveolus of lung, 0.96 conf). `Brain` → `UBERON:0001894` (diencephalon, 0.96 conf). `iPS cells` → 10× specific `iPS-Nn` cell-line variants. The abstract parent (`UBERON:0002048`, `UBERON:0000955`, `CL:0002248`) never makes the top-K because every narrow descendant outscores it on label-only cosine.
  - **Planned fix (A first, bigger win):** at `caom.retrieval.index.build_index` time, filter `terms_df` to an allow-list of CURIE prefixes that are legitimate ChIP-Atlas contexts: `CL`, `UBERON`, `EFO`, `CLO`, `BTO`, `Orphanet`, `MONDO`, plus the organism-specific anatomy branches we actually need (`FBbt`, `ZFA`, ...). Exclude `PR`, `HGNC`, `OBA`, `GO`, `NCBITaxon`, `HP` (phenotype, not context), and the `dbpedia:*` geographic terms. Document the allow-list inline so future ontology refreshes don't silently re-introduce contamination.
  - **Planned fix (B):** query augmentation — prepend `cell_type_class` (and/or a coarsened form of `cell_type`) so broad queries like `Lung` retrieve lung-level terms; optionally re-rank by parent subsumption (boost candidates whose parent is also in the top-K, reducing subtype dominance). Re-measure on the same 7-row smoke sample + the full Ikeda gate after each change.
  - Gate: rebuild FAISS index after each change (`caom.update_ontologies(force=True)` or a cheaper index-only rebuild). Expected post-Stage-7 state: the 7-row smoke sample maps `Pancreatic islets`, `CD4+ T cells`, `iPS cells`, `Lung`, `Brain` to their canonical IDs (or at worst a defensible parent/child), and the full Ikeda gate posts a real acc@1 number instead of deferring to Stage 8.
- **Stage 8: full-pipeline threshold tuning** — after Stage 7
  - Once retrieval returns a plausible candidate set, run `CAOM_RUN_LLM_VALIDATION=1 pytest tests/validation` to establish the full-pipeline baseline, then iterate on prompt wording + LLM abstention threshold + top-K to close the gap vs. Stage 2's `acc@1=0.903` cell-line baseline. Tighten the validation gate floors once the numbers stabilize.
