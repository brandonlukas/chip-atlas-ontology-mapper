...
- **Stage 10: residual retrieval gaps + LLM-model-baseline reset** — NEXT
  - **Goal.** Close the two residuals surfaced at Stage 9 closeout (plural / asymmetric variants kill the exact layer) and re-anchor the Ikeda baseline back to `qwen2.5:7b-instruct`.
  - **Approach — in expected value-to-risk order. Evaluate (A) before committing to (B)/(C).**
    1. **Substring-preference fallback** (Stage 9 step 4, carried over). The smallest and cheapest. In `caom/llm/prompts.py`, add a rule: *"If the query is a case-insensitive substring of a candidate's label or one of its synonyms (or vice-versa), prefer that candidate over a more specific subtype, anatomical substructure, or unrelated term — unless the disambiguation context clearly contradicts."* Verify on the matcha smoke gate: expect `Pancreatic islets → UBERON:0000006` (canonical islet of Langerhans has synonym `pancreatic islet`, substring of the query) and `CD4+ T cells → CL:0000624` (label `CD4-positive, alpha-beta T cell` contains `CD4 T cell` as a substring ... actually, prob needs tokenization — test before locking the claim). No FAISS rebuild; just prompt + matcha gate re-anchor.
    2. **Corpus refresh.** Retry `caom.update_ontologies(force=True)` against a newer EFO (`>3.89`) or pull CL / UBERON / MONDO directly to pick up `CL:2000001` (PBMC), `CL:0011020` (neural progenitor), `CL:0008007` (induced pluripotent stem cell). Any of these would let the matcha gate tighten from "corpus-best parent" to "canonical term". If the refresh doesn't include them, consider falling back to a thin direct-CL-pull step in `update_ontologies` — but that veers toward "generalizing outside ChIP-Atlas" (see anti-goal), so only do it if the EFO import is provably lagging.
    3. **BM25 / FTS second layer** (mochi's middle tier, previously Stage 9-deferred). Biggest-impact retrieval change: a SQLite FTS5 table persisted alongside the FAISS index, queried as tier 2 between exact-match (tier 1) and vector (tier 3). The `[bm25]` marker in the prompt would parallel `[exact]`. Only attempt if (1) still leaves the matcha gate at two+ residuals — the exact layer already captures the low-hanging lexical signal.
    4. **LLM-model-baseline reset.** Pull `qwen2.5:7b-instruct` locally, re-run `test_accuracy.py::test_full_pipeline_accuracy_gate` to re-anchor the published baseline, and update the module docstring in `tests/validation/test_accuracy.py` with the new observed numbers. This is non-optional if Stage 10 lands prompt changes — the gate floors are calibrated to the 7b pick distribution, not 14b's.
  - **Deliberately out of scope (deferred to Stage 11+).**
    - **Embedder upgrade** (`pritamdeka/S-PubMedBert-MS-MARCO` → E5 / nemotron-family with asymmetric `query:` / `passage:` prefixes). Own FAISS rebuild; orthogonal to the exact + BM25 + corpus work.
    - **Vector-store swap** (FAISS → LanceDB). Architecture, not a retrieval-quality lever; revisit when FAISS becomes operationally inconvenient rather than for gate-score reasons.
    - **Ontology-scope expansion.** The current `CL + UBERON + MONDO` + organism-anatomy set covers the matcha / Ikeda shapes. Don't add new prefixes speculatively — let real-data coverage gaps drive it (see the allow-list comment).
  - **Anti-goals (carry over + reinforced).** No fuzzy matching in the Cellosaurus fast-path. No asymmetric query rewriting — even the substring rule (1) must match symmetrically via `in` on normalized strings on both sides. No relaxing the LLM-id allowlist. No silently dropping unmappable rows. No generalizing `caom` outside ChIP-Atlas scope — in particular, no introducing a direct-ontology-pull step that isn't EFO-gated unless the EFO import is demonstrably lagging. No abandoning the Cellosaurus separate-source split.
  - **Reproduce the evidence for the open residuals.** `CAOM_LLM_MODEL=qwen2.5:14b CAOM_RUN_LLM_VALIDATION=1 pytest tests/validation/test_matcha_smoke.py` — the two rows with the `LLM_PICK — known-imperfect` tag in the EXPECTED dict are the residuals step (1) targets.
